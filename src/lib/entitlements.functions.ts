...
export const getMyAccess = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<AccessSummary | null> => {
    // Never throw at the caller: a misconfigured service key must degrade to
    // "no access information", not a broken screen.
    try {
      const { resolveAccess } = await import("@/lib/entitlements.server");
      return await resolveAccess(context.userId);
    } catch (e) {
      console.error("[entitlements] getMyAccess failed", e);
      return null;
    }
  });
